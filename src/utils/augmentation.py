...
def get_training_augmentation(scale_limit=(0.8, 2.0)):
    train_transform = [
        ...
        albu.ShiftScaleRotate(scale_limit=0.0, rotate_limit=15, p=1, border_mode=0),
        albu.PadIfNeeded(
            min_height=512,
            min_width=512,
            always_apply=True,
...
def copyblob(
    dst_img, dst_mask, meta_df, paste_list, label2num, get_preaug, get_postaug
):
    ...
    pre_src_cnt = np.sum(src_mask == src_class) / (
    ...
    if src_cnt != 0 and dst_cnt != 0:
        ...
        src_idx_sum = list(
            src_idx[0][i] + src_idx[1][i] for i in range(len(src_idx[0]))
        )
        ...
        topleft_idx = np.where(
            (dst_idx[0] <= np.percentile(dst_idx[0], 50))
