...
def calc_kmeans_silhouette(df, cols, wo_bg_border):
    ...
    for cur_num in range(2, 30):
        km = KMeans(n_clusters=cur_num, init='k-means++', n_init=50, max_iter=1000, random_state = 2022)
        km.fit(X)
...
def apply_cluster(df, X, num_cluster, kmeans_exp_list, labels, num2color, figloc, wo_bg_border):
    if wo_bg_border:
        labels = labels[:-2]
    ...
    color = ['#%02x%02x%02x' % (int(r),int(g),int(b)) for r,g,b in num2color.values()]
    ax = cluster_center_df.plot(kind='bar', stacked=True, figsize=(12, 6),
                               color=color,
                               edgecolor='black')
...
if __name__ == '__main__':
    ...
    meta_df['major_label'] = meta_df['major_label'].apply(lambda x: x.replace('_sum', ''))
    ...
    reduce_classes = "bird, cat, aeroplane, bicycle, bottle, chair".replace(',',' ').split()
    ...
    for col in reduce_classes:
        ...
        train_col_id = meta_df[(meta_df.ftype == 'train') & (meta_df[col] == True) & (meta_df['use_image']==True)].id
        train_num = len(train_col_id)
        remove_id = np.random.choice(list(train_col_id), int(train_num*remove_prop), replace=False)
    ...
    meta_df = apply_cluster(meta_df, X, num_cluster, exps[0][0], list(labels), num2color, f'./data/trainval_cluster_{num_cluster}.png', True)

    train_df = meta_df[(meta_df['ftype'] == 'train') & (meta_df['use_image'] ==True )]
    X = train_df[label_prop_cols[:-2]].values
    train_df = apply_cluster(train_df, X, num_cluster, exps[0][0], list(labels), num2color, f'./data/train_cluster_{num_cluster}.png', True)
    ...
    for rs in range(1,6):
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(train_df.index, train_df.cluster_num_16)):        
            train_df.loc[train_df.index.isin(train_idx), f'skf_prop_rstate{rs}_fold{fold}'] = 'train'
            train_df.loc[train_df.index.isin(val_idx), f'skf_prop_rstate{rs}_fold{fold}'] = 'val'        
